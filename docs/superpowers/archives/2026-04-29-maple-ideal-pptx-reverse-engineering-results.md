...
`shared-tasks/maple-ideal-reverse-20260429-153743`
...
- `TASK_DIR=shared-tasks/maple-ideal-reverse-20260429-153743 CODEX_AGENT_TIMEOUT_SECONDS=120 PYTHONPATH=apps/worker/src apps/worker/.venv/bin/python` running `run_pdf_to_ppt(Path(os.environ["TASK_DIR"]))`
  - Result: exited `0`, wrote `candidate.v1.pptx`, `candidate.v2.pptx`, `final.pptx`, `validator.v1.json`, and `validator.v2.json`
- `TASK_DIR=shared-tasks/maple-ideal-reverse-20260429-153743 PYTHONPATH=apps/worker/src apps/worker/.venv/bin/python` running `compare_pptx_structure(output/final.pptx, Maple ideal PPTX)`
